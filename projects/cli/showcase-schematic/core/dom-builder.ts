...
import { File, join, toRelativePath } from '../../../core';
import { Cell, DomData, RenderedCell } from './dom-builder-model';
...
export abstract class DomBuilder {
  ...
  relativeContainer = this.document.createElement('div');
  flexContainer = this.document.createElement('div');
  left = this.document.createElement('div');
  right = this.document.createElement('div');
  ...
  getTitles(): HTMLElement[] {
    return Array.from(this.document.querySelectorAll<HTMLElement>('h1, h2, h3, h4'));
  }
  ...
  createFile(
    partialFile: Omit<File, 'extension' | 'fileNameWithoutExtension' | 'extensionName' | 'relativePath' | 'localPath'>
  ): File {
    ...
    return {
      ...
      extensionName: parsedFileName.ext.slice(1),
      relativePath: toRelativePath(join(partialFile.id, fileNameWithoutExtension)),
  ...
  protected addCommon(): void {
    const addLayout = () => {
      ...
      this.setAttribute(this.body, '[class.berg-showcase-mobile]', 'isMobile$ | async');
    ...
    const addTableOfContents = () => {
      const tocLevels: Record<string, number> = {
        H1: 1,
        H2: 2,
        H3: 3,
        H4: 4,
      ...
      for (const title of this.getTitles()) {
        ...
        item.classList.add(`berg-showcase-table-of-contents-level-${tocLevels[title.tagName]}`);
        item.textContent = title.childNodes[0].textContent;

        this.setAttribute(item, '(click)', `updateLocationWithId('${id}', true)`);
        this.setAttribute(item, '[class.berg-showcase-active-item]', `isActiveRoute('${id}')`);
    ...
    const addLinkButtons = () => {
      for (const title of this.getTitles()) {
        ...
        this.setAttribute(buttonElement, '(click)', `updateLocationWithId('${id}')`);
    ...
    const addIntersectionObserverNodes = () => {
      for (const title of this.getTitles()) {
        ...
        this.setAttribute(title, '(bergIntersectionNodeChange)', `onIntersectionChanged($event, '${id}')`);
      }
    };

    const addRouterLinks = () => {
      const links = Array.from(this.document.querySelectorAll<HTMLElement>('a'));
    ...
    const addHighlighting = () => {
      for (const element of Array.from(this.body.querySelectorAll('pre > code'))) {
  ...
  protected setAttribute(element: HTMLElement, attribute: string, value = ''): void {
    element.setAttributeNS(null, attribute, value);
  }

  protected appendCode(code: string): void {
    this.body.appendChild(this.createElementWithContent('p', this.createElementWithContent('code', code)));
  }
  ...
  protected appendTable<T>(cells: Cell<T>[], items: T[]): void {
    ...
    const [tableElement, tableHeadElement, tableBodyElement] = this.createTableElement();
    ...
    tableHeadElement.appendChild(this.createRowElement(renderedHeaderCells, 'th'));
  ...
  protected createElementWithContent(tagName: string, content: string | null | HTMLElement = null): HTMLElement {
  ...
  protected createIconButtonElement(iconName: string, tagName: 'a' | 'button' = 'button'): HTMLElement {
    ...
    this.setAttribute(icon, 'svgIcon', iconName);
  ...
  protected createRowElement(renderedCells: RenderedCell[], cellType: 'td' | 'th' = 'td'): HTMLElement {
    ...
    for (const cell of renderedCells) {
      ...
      const contents = Array.isArray(cell.content) ? cell.content : [cell.content];
