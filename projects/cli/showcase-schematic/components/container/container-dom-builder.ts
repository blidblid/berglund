...
export class ContainerDomBuilder extends DomBuilder {
  constructor(protected jsdom: JSDOM, private context: Context, private children: Component[]) {
    super(jsdom);
  ...
  private createNavBarElement(): HTMLElement {
    ...
    for (const showcaseComponent of [...this.children].reverse()) {
      ...
      this.setAttribute(a, '(click)', `navigateToChild('${showcaseComponent.name}')`);
      this.setAttribute(a, '[active]', `routeIncludesName('${showcaseComponent.name}')`);
